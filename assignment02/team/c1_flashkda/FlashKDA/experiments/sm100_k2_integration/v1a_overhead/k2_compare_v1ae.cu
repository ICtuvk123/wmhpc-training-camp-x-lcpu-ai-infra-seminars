// Compare the real production K2 baseline and V1a in one process. Capture each
...
int main(int argc, char** argv) {
    ...
    if (tokens % 16 != 0) { std::fprintf(stderr, "tokens must be divisible by 16\n"); return 2; }
    ...
    Buffer<BF16> initial(state_elements), out_base(elements), out_v1a(elements), out_v1ae(elements),
                 final_base(state_elements), final_v1a(state_elements), final_v1ae(state_elements);
    ...
    CHECK(cudaMemset(out_v1a.ptr, 0xff, elements * sizeof(BF16)));
    CHECK(cudaMemset(out_v1ae.ptr, 0xff, elements * sizeof(BF16)));
    CHECK(cudaMemset(final_base.ptr, 0xff, state_elements * sizeof(BF16)));
    CHECK(cudaMemset(final_v1a.ptr, 0xff, state_elements * sizeof(BF16)));
    CHECK(cudaMemset(final_v1ae.ptr, 0xff, state_elements * sizeof(BF16)));
    ...
    auto v1a = [&]() { launch_fwd<128, true, true, false, false, false, true>(
        q.ptr, k.ptr, v.ptr, g.ptr, beta.ptr, initial.ptr, 1.f / std::sqrt(float(D)),
        final_v1a.ptr, out_v1a.ptr, workspace.ptr, tiles, total, H, batch, nullptr,
        alog.ptr, bias.ptr, -5.f * 1.4426950408889634f, stream); };
    auto v1ae = [&]() { launch_fwd<128, true, true, false, false, false, false, 1>(
        q.ptr, k.ptr, v.ptr, g.ptr, beta.ptr, initial.ptr, 1.f / std::sqrt(float(D)),
        final_v1ae.ptr, out_v1ae.ptr, workspace.ptr, tiles, total, H, batch, nullptr,
        alog.ptr, bias.ptr, -5.f * 1.4426950408889634f, stream); };
    ...
    CHECK(cudaStreamBeginCapture(stream, cudaStreamCaptureModeGlobal)); v1a();
    CHECK(cudaStreamEndCapture(stream, &graphs[1].full)); graphs[1].extract();
    CHECK(cudaStreamBeginCapture(stream, cudaStreamCaptureModeGlobal)); v1ae();
    CHECK(cudaStreamEndCapture(stream, &graphs[2].full)); graphs[2].extract();
    for (auto& graph : graphs) CHECK(cudaGraphLaunch(graph.exec, stream));
    CHECK(cudaStreamSynchronize(stream));
    const bool correct = equal(out_base.download(), out_v1a.download(), "v1a output") &&
                         equal(final_base.download(), final_v1a.download(), "v1a final_state") &&
                         equal(out_base.download(), out_v1ae.download(), "v1ae output") &&
                         equal(final_base.download(), final_v1ae.download(), "v1ae final_state");
    if (!correct) { std::puts("{\"correct\":false}"); return 4; }
    cudaEvent_t start, stop; CHECK(cudaEventCreate(&start)); CHECK(cudaEventCreate(&stop));
    double sums[3] = {0, 0, 0};
    const char* names[3] = {"baseline", "v1a", "v1a_staged_egress"};
    for (int round = 0; round < rounds; ++round) for (int order = 0; order < 3; ++order) {
        const int impl = round % 2 ? 2 - order : order;
        ...
        std::printf("{\"implementation\":\"%s\",\"correct\":true,\"B\":%d,\"T\":%d,\"H\":64,\"D\":128,"
                    ...
                    names[impl], batch, tokens, round + 1, us,
                    graph.attributes.numRegs, graph.dynamic_smem, graph.attributes.localSizeBytes);
        std::fflush(stdout);
    }
    if (!equal(out_base.download(), out_v1a.download(), "timed v1a output") ||
        !equal(final_base.download(), final_v1a.download(), "timed v1a final_state") ||
        !equal(out_base.download(), out_v1ae.download(), "timed v1ae output") ||
        !equal(final_base.download(), final_v1ae.download(), "timed v1ae final_state")) return 4;
    std::printf("{\"summary\":true,\"chunks\":%d,\"baseline_k2_us\":%.6f,"
                "\"v1a_k2_us\":%.6f,\"v1a_staged_egress_k2_us\":%.6f,"
                "\"staged_recovery_us\":%.6f}\n",
                tokens / chunk, sums[0] / rounds, sums[1] / rounds, sums[2] / rounds,
                (sums[1] - sums[2]) / rounds);

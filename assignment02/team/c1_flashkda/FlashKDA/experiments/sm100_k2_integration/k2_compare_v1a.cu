...
int main(int argc, char** argv) {
    ...
    Buffer<BF16> initial(state_elements), out_base(elements), out_v1a(elements), final_base(state_elements), final_v1a(state_elements);
    ...
    const bool correct = equal(out_base.download(), out_v1a.download(), "output") &&
                         equal(final_base.download(), final_v1a.download(), "final_state");
    ...
    for (int round = 0; round < rounds; ++round) for (int order = 0; order < 2; ++order) {
        ...
        std::printf("{\"implementation\":\"%s\",\"correct\":true,\"B\":%d,\"T\":%d,\"H\":64,\"D\":128,"
                    ...
                    impl ? "v1a" : "baseline", batch, tokens, round + 1, us,
                    graph.attributes.numRegs, graph.dynamic_smem, graph.attributes.localSizeBytes);
        std::fflush(stdout);
    }
    if (!equal(out_base.download(), out_v1a.download(), "timed output") ||
        !equal(final_base.download(), final_v1a.download(), "timed final_state")) return 4;
    std::printf("{\"summary\":true,\"baseline_k2_us\":%.6f,\"v1a_k2_us\":%.6f,\"speedup\":%.6f}\n",

// P1 epilogue control: compute K^T U in TMEM, then add FP32 S in registers.
//
...
// Layout and per-CTA state contract match tcgen05_p1.cu.
__global__ void tcgen05_p1_epi_kernel(const BF16* __restrict__ kt,
...
int main(int argc, char** argv) {
  auto launch = [](int batch, const BF16* kt, const BF16* u, const float* s, float* c) {
    tcgen05_p1_epi_kernel<<<batch, kThreads, kDynamicSmemBytes>>>(kt, u, s, c);
  };
  return run_p1(argc, argv, "tcgen05_p1_epi", tcgen05_p1_epi_kernel, launch, kTmemColumns);
}

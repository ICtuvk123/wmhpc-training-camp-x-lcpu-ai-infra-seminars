// P2: compute K^T U in TMEM, then add FP32 S * g_total in registers.
//
...
// g_total is FP32 [batch, kM], broadcast over output columns.
__global__ void tcgen05_p2_epi_kernel(const BF16* __restrict__ kt,
...
#pragma unroll
  ...
  const float* block_g = g_total + static_cast<size_t>(blockIdx.x) * kM;
...
#pragma unroll
  for (int i = 0; i < size(tDrAcc); ++i) {
    tDrAcc(i) = tDrS(i) * tDgG(i) + tDrAcc(i);
  }
  copy(tDrAcc, tDgC);
...
int main(int argc, char** argv) {
  auto launch = [](int batch, const BF16* kt, const BF16* u, const float* s,
    ...
    tcgen05_p2_epi_kernel<<<batch, kThreads, kDynamicSmemBytes>>>(kt, u, s, g, c);
  };
  return run_p2(argc, argv, "tcgen05_p2_epi", tcgen05_p2_epi_kernel, launch, kTmemColumns);
}
